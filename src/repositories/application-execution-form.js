"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
exports.__esModule = true;
var Sequelize = require("sequelize");
var index_1 = require("../models/index");
var database_1 = require("../bootstrap/database");
exports.getByApplicationExecutionId = function (applicationExecutionId) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.findAll({
                attributes: ['id', 'applicationExecutionId', 'applicationFormFieldId', 'value', 'createdAt', 'updatedAt'],
                where: {
                    isActive: true,
                    applicationExecutionId: applicationExecutionId
                },
                include: [{
                        model: index_1.Models.ApplicationFormField,
                        where: {
                            isActive: true
                        }
                    }]
            })];
    });
}); };
exports.getByApplicationExecutionIdAndFieldId = function (applicationExecutionId, fieldId) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.findOne({
                attributes: ['id', 'applicationExecutionId', 'applicationFormFieldId', 'value', 'createdAt', 'updatedAt'],
                where: {
                    isActive: true,
                    applicationExecutionId: applicationExecutionId,
                    fieldId: fieldId
                }
            })];
    });
}); };
exports.getExecutionIdsByStartEndDate = function (applicationId, startDate, endDate, status) { return __awaiter(_this, void 0, void 0, function () {
    var query, result;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                query = "\n        select distinct execution.id, form.\"applicationFormFieldId\", form.value\n        from \"applicationExecutionForm\" form\n        inner join \"applicationExecution\" execution on form.\"applicationExecutionId\" = execution.id\n        inner join \"applicationFormField\" field on field.id = form.\"applicationFormFieldId\"\n        where execution.\"applicationId\" = '" + applicationId + "' and field.\"templateName\" = 'input_attachment'\n        and execution.\"createdAt\" >= '" + startDate + "' and execution.\"createdAt\" < '" + endDate + "'\n    ";
                if (status && status !== 'all') {
                    query += " and execution.status = '" + status + "'";
                }
                return [4 /*yield*/, database_1.Database.query(query).then(function (res) { return res[0]; })];
            case 1:
                result = _a.sent();
                return [2 /*return*/, result];
        }
    });
}); };
exports.findById = function (id) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.findOne({ where: { id: id } })];
    });
}); };
exports.findByIds = function (ids) { return __awaiter(_this, void 0, void 0, function () {
    var _a;
    return __generator(this, function (_b) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.findAll({ where: { id: (_a = {}, _a[Sequelize.Op["in"]] = ids, _a) } })];
    });
}); };
exports.saveApplicationExecutionForm = function (applicationExecutionForm) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.upsert(applicationExecutionForm, { returning: true })
                .then(function (res) { return res[0]; })];
    });
}); };
exports.deleteApplicationExecutionForm = function (id) { return __awaiter(_this, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/, index_1.Models.ApplicationExecutionForm.update({ isActive: false }, { where: { id: id } })];
    });
}); };
